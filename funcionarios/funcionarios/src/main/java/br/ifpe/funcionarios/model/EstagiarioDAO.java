...
public class EstagiarioDAO {
    ...
    // Filtro
    public List<Estagiario> filtrarPorNome(String nome) {
        String sql = "SELECT * From funcionario where cargo = 'Estagiario' AND nome LIKE :nome";
        return jdbc.sql(sql)
                .param("nome", "%" + nome + "%")
                .query(this::mapRow)
                .list();
    ...
    public Integer update(Estagiario e) {
        String sql = """
                    UPDATE funcionario SET
                        ...
                        salario = :salario, rua = :rua, cidade = :cidade, instituicao = :instituicao
                    Where id = :id
                """;
        return jdbc.sql(sql)
                .param("nome", e.getNome())
    ...
    private Estagiario mapRow(ResultSet r, int rowNum) throws SQLException {
        Estagiario e = new Estagiario(
                ...
                r.getDouble("salario"), r.getString("rua"), r.getString("cidade"), r.getString("instituicao"));
        e.setId(r.getInt("id"));
        return e;
    }
}